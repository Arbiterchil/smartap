 ...
 */
public class attenLts extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        s =  inflater.inflate(R.layout.fragment_atten_lts, container, false);
    ...
    private void sea(final String seacrh) {
        ...
        fear.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                DatabaseReference me = fear.child(hahawe).child("LateRecord");

                Query query = me.orderByChild("fullname").startAt(seacrh).endAt(seacrh+"\uf8ff");

                FirebaseRecyclerOptions<getLa> options = new FirebaseRecyclerOptions.Builder<getLa>()
                        .setQuery(query,getLa.class)
                        .build();
               FirebaseRecyclerAdapter<getLa,Kingdom> adapter = new FirebaseRecyclerAdapter<getLa, Kingdom>(options) {
                   @Override
                   protected void onBindViewHolder(@NonNull Kingdom holder, int position, @NonNull getLa model) {

                       holder.fn.setText(model.getFullname());
                       holder.sta.setText(model.getDate());
                       holder.uid.setText(model.getLatetimes());
                   ...
                   public Kingdom onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
                       View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.abslats,viewGroup,false);
                       return new Kingdom(view);
                   }
               };
